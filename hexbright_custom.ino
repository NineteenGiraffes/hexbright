/* 
  ...
  Custom firmware for HexBright FLEX
...
#define MODE_OFF                0
#define MODE_LOW                1
#define MODE_LOW_TO_MED         2
#define MODE_MED                3
#define MODE_MED_TO_HIGH        4
#define MODE_HIGH               5
...
void blinkOnCharge(unsigned long time)
{
  ...
  {
    flashlight.setRedLED(time & 0x0100);
  }
  else if (chargeStatus > 768) // Charged: steady
  {
    flashlight.setRedLED(false);
...
void loop()
{
  ...
  case MODE_OFF:
    if (btnDown && !newBtnDown && (time-btnTime)>20)
      newMode = MODE_LOW;
    if (btnDown && newBtnDown && (time-btnTime)>150)
      newMode = MODE_HIGH;
    break;
  case MODE_LOW:
    ...
    if (btnDown && newBtnDown && (time-btnTime)>150)
      newMode = MODE_LOW_TO_MED;
    break;
  case MODE_LOW_TO_MED:
  ...
  case MODE_MED:
    ...
    if (btnDown && newBtnDown && (time-btnTime)>150)
      newMode = MODE_MED_TO_HIGH;
    break;
  case MODE_MED_TO_HIGH:
    if (btnDown && !newBtnDown)
      newMode = MODE_HIGH;
    break;
  case MODE_HIGH:
